...
PixelInputType VS_Main(VertexInput input) 
{
	...
	float2 waveCoords = input.Texcoord * float2(3.5, 3.5) + float2(uTimeSinceStart, uTimeSinceStart) * float2(0.0001, 0.0001);
	
    output.Position = mul(output.Position, input.WMatrix);
	output.Position.y += snoise(waveCoords) * 2;
	output.Position.xz += float2((snoise(waveCoords)-0.5f) * 15 , (snoise(-waveCoords)-0.5f) * 15);
	...
	float heightL = snoise(float2(waveCoords.x-1, waveCoords.y));
	float heightR = snoise(float2(waveCoords.x+1, waveCoords.y));
	float heightD = snoise(float2(waveCoords.x , waveCoords.y-1));
	float heightU = snoise(float2(waveCoords.x , waveCoords.y+1));
	...
	output.Color = float3(0, 201.0/255.0, 1);
